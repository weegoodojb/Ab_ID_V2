...
- 단일항체로 설명되지 않으면 2항체 조합(예: anti-D + anti-E)이 전체 패턴을 완벽히 설명하는지 확인해 `COMPOUND_CANDIDATE`로 표시합니다. 3개 이상 조합은 아직 포함하지 않으며, 조합 결과도 `CONFIRMED`가 아닌 검사자 확인이 필요한 후보로만 표시됩니다.
- AHG 반응 강도가 섞여 있으면(예: 일부 셀만 4+, 나머지는 3+) 가장 강한 반응 셀과 정확히 일치하는 항원을 앵커로 우선 선정하고, 나머지 약한 양성 셀은 조합으로 설명합니다.
- 확정/후보로 제시된 항체에 대해 환자 자신의 항원검사 결과를 입력하면, 환자가 그 항원 양성인데 해당 항체가 후보로 나온 경우 모순 경고를 표시합니다.